...
### Q15. CPU가 메모리 속 프로그램을 순차적으로 읽어 들이고 실행해 나갈 수 있는 이유는 _________ 쭈군히 증가하기 때문이다. 빈칸에 들어갈 키워드를 쓰시오.
...
- 프로그램 카운터는 메모리로 부터 데이터를 읽어들이고나면, 프로그램 카운터를 증가시켜, 다음 명령어를 읽어들일 준비를 한다.
