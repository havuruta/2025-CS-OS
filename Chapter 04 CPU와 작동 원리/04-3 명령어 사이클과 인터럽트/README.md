...
### Q6. 다음 중 디버깅 모드를 사용할 때 발생하는 동적 인터럽트(예외)는?
...
- 디버깅 모드는 특정 코드가 실행된 순간의 모습을 보기 위해 멈춤
	- 멈춘 동작이 예외
- 이후 다음 코드부터 동작
- 즉, 트랩이 발생함
...
- 폴트(Fault)
	...
	- 꼭 필요한 데이터가 보조저장장치에 있는 경우 예외 발생시키고 다녀옴
	-  돌아왔으니까 예외 발생시킨 자리에서부터
- 트랩(Trap)
	- 예외를 처리한 직후 예외가 발한 명령어의 다음 명령어부터
	-  디버깅 할 때, 특정 코드 실행된 순간의 모습을 보기 위해 실행을 잠깐 멈춤
- 중단(Abort)
...
### Q7. 다음 중 인터럽트 플래그에 대한 내용으로 옳지 않은 것은?
...
### Q8. 다음 중, 명령어 사이클에 관한 설명으로 옳지 않은 것은?
