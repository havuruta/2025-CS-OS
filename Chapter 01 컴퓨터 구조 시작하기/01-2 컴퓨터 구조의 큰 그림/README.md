...
#### X
주기억장치는 실행되는 프로그램의 명령어와 데이터를 저장하지만, 전원이 꺼지면 저장된 내용을 잃는다다. 주기억장치는 메모리라고도 한다.

</details>


### Q8. CPU가가 메모리에 어떤 값을 저장하고 싶을 땐 메모리를 향해 메모리 읽기 라는 신호를 보낸다. (O / X)
...
CPU가가 메모리에 어떤 값을 저장하고 싶을 땐 메모리를 향해 메모리 쓰기기 라는 신호를 보낸다.
